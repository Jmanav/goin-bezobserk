...
- [x] **B0.2 — `candidate_pairs.tsv` schema (Q7, research.md §11).** Owner D owns this question, but B produces the file's content. io_rules.md §5.2 states: same schema as `matching_results.tsv` — `source1_entity_id`, `candidate_entity_ids` — same formatting rules. Research.md §11 Q7 records a competing default ("two columns S1 ID, fragment ID"). **RESOLVED: io_rules.md §5.2, the S1-row-with-comma-list form.** **Evidence (2026-09-25):** the organiser's own `train_ground_truth.tsv` has header `source1_entity_id<TAB>matched_entity_ids` with n_data_rows == n_distinct_s1 == 2,206,821 — i.e. the io_rules.md §5.1 list shape, one row per S1. That settles Q7 in io_rules.md's favour; research.md §11's "two columns S1 ID, fragment ID" default is wrong.
